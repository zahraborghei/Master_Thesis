...
class Blip2OPT(Blip2Base):
    ...
    def __init__(
        ...
        # Calculate the actual output size after compression and flattening
        num_compression_channels = int(round(2048 / (self.final_spatial**2)))  # This is 32
        actual_output_size = num_compression_channels  # 32 after flattening

        # Now create image_fc after output_size is available
        self.image_fc = nn.Sequential(
            # nn.Linear(self.output_size, 1024), # 32 -> 1024
            nn.Linear(actual_output_size, 1024), 
            nn.ReLU(),
            nn.Linear(1024, 768)# 1024 -> 768
        )
    ...
    def forward(self, samples):
        ...
        # Reshape embeddings from [batch_size, 286720] to [batch_size, 286720, 1, 1]
        # This treats the full embedding as channels with 1x1 spatial dimensions
        batch_size = image_embeddings.shape[0]
        image_embeddings = image_embeddings.view(batch_size, -1, 1, 1)
